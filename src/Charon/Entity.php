...
abstract class Entity
{
    ...
    public function __set($key, $value)
    {
        ...
        $setter = preg_replace("[^a-zA-Z0-9]", '', $key);
        $setter = "set" . ucfirst($setter);

        if ($this->getMetadata()->hasMethod($setter)) {
            $this->{$setter}($value);
        } else if ($this->getMetadata()->hasMethod($key)) {
            $this->{$key}($value);
    ...
    public function loadValues($fields)
    {
...
//                    echo "{$this->table}.{$key} => {$fields[$key]} \n";
    ...
    public function find($id, $attachRK = false)
    {
    ...
    public function setJsonDecorator(JsonDecorator $jd)
    {
